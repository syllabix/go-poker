...
func BenchmarkRevealBestHand(b *testing.B) {
	for _, test := range tests {
		b.Run(test.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				RevealBestHand(test.args.input)
			}
		})
	}
}
...
var tests = []struct {
	...
	{
		name:    "Full House From the Deck",
		args:    args{"2H 3S 2D QD JC TC TS TD 8D 9D"},
		want:    "Hand: 2H 3S 2D QD JC Deck: TC TS TD 8D 9D Best hand: full-house",
		wantErr: false,
	},
	{
		name:    "Straight Flush",
